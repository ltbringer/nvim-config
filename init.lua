...
do
  ...
  vim.o.number = true
  -- Relative line numbers (hybrid: absolute on the current line, relative on
  -- the rest) to make `10j` / `5k` style jumps easy.
...
do
  ...
  local servers = {
    ...
    -- Python: pyright for types/completion/hover, ruff for lint + (via conform) format.
    -- Point pyright at the project's uv-managed .venv so imports (httpx, etc.)
    -- resolve. Computed per-project from the LSP root, so it works everywhere
    -- without a pyrightconfig.json.
    pyright = {
      before_init = function(params, config)
        local root = config.root_dir
        if not root and params and params.rootUri then
    ...
    ruff = {
      -- Let pyright own hover so we don't get duplicate hover windows.
...
do
  ...
  local parsers = { 'bash', 'c', 'diff', 'html', 'lua', 'luadoc', 'markdown', 'markdown_inline', 'python', 'query', 'toml', 'vim', 'vimdoc' }
